...
namespace Clouded.Core.DataSource.Mysql;
...
public class Context(Connection connection) : DbContext, IContext
{
    ...
    private readonly string[] _prohibitedSchemas = { "information_schema" };
    ...
    public async Task<string> GetDatabaseSchema()
    {
        ...
        var dumpResult = await Cli.Wrap("mysqldump")
            .WithArguments(
                new[]
                {
                    "-h",
                    HttpUtility.UrlEncode(connection.Server),
                    "-P",
                    connection.Port.ToString(),
                    "-u",
                    HttpUtility.UrlEncode(connection.Username),
                    "-p",
                    connection.Password,
                    "--databases",
                    connection.Database
    ...
    public IEnumerable<string> GetSchema()
    {
        var schema = Select<DataSourceDictionary>(
            new SelectInput
            {
                ...
                {
                    new() { ColJoin = new[] { "schema", "schema_name" } }
                }
            }
        );
        ...
        try
        {
            CreateTable(
                new TableInput
                {
                    Schema = "public",
                    Name = tableName,
                    Columns = new List<ColumnInput>
                    {
                        new() { Name = "test", Type = EColumnType.Boolean, }
                    }
                }
            );
                    ...
                    .Select(
                        parameter =>
                            parameter.Value is not object[] objects
                                ? new MySqlParameter(parameter.Key, parameter.Value ?? DBNull.Value)
